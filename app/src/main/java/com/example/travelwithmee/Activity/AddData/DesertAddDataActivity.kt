...
import com.example.travelwithmee.databinding.ActivityDesertAddDataBinding
import com.google.firebase.database.FirebaseDatabase
import com.google.firebase.storage.FirebaseStorage
import com.google.firebase.storage.StorageReference
import java.util.UUID

class DesertAddDataActivity : AppCompatActivity() {
    lateinit var binding: ActivityDesertAddDataBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityDesertAddDataBinding.inflate(layoutInflater)
    ...
    private fun initview() {
        binding.btnbsubmit.setOnClickListener {
            ...
            var key = reference.root.child("DesertDataTb").push().key ?: ""

            var modelClass =
                CategoryModelclass(place, city, description, price, people, key, Imageuri,Like)

            reference.root.child("DesertDataTb").child(key).setValue(modelClass)
    ...
    private fun uploadImage() {
        if (uri != null) {
            ...
            val ref: StorageReference = FirebaseStorage.getInstance().getReference()
                .child("images/Desert/" + UUID.randomUUID().toString())
